...
contract DebtAllocator is Ownable {
    ...
    function updateRewardsConfig(address _rewardsPayer, address _rewardsStreamer, uint216 _rewardsPerSec) external onlyOwner {
        ...
        require(IStreamer(_rewardsStreamer).streamToStart(streamId) > 0, "STREAM");
    ...
    function updateStaleSnapshotPeriod(uint256 _staleSnapshotPeriod) external onlyOwner {
    ...
    function addStrategy(
        ...
        if(strategiesHash != 0){         
            require(strategiesHash == uint256(keccak256(abi.encodePacked(_packedStrategies.addresses, _packedStrategies.callLen, _packedStrategies.contracts, checkdata, _packedStrategies.offset, _packedStrategies.calculationsLen, _packedStrategies.calculations, _packedStrategies.conditionsLen, _packedStrategies.conditions))), "DATA");   
        } else {
...
//
    ...
    function verifySolution(uint256[] memory programOutput) external returns(bytes32){
        ...
        require(targetAllocation.length == current_target_allocation.length && targetAllocation.length == new_target_allocation.length,"TARGET_ALLOCATION_LENGTH");
